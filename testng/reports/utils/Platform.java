...
public class Platform {
	...
	public static void prepareDetails(WebDriver paramWebDriver) {
		...
		try {
			String str = (String) ((JavascriptExecutor) paramWebDriver)
					.executeScript("return navigator.userAgent;", new Object[0]);
			if (str.contains("MSIE")) {
				BROWSER_VERSION = str.substring(str.indexOf("MSIE") + 5,
						str.indexOf("Windows NT") - 2);
				BROWSER_NAME = "Internet Explorer";
			} else if (str.contains("Firefox/")) {
				BROWSER_VERSION = str.substring(str.indexOf("Firefox/") + 8);
				BROWSER_NAME = "Mozilla Firefox";
			} else if (str.contains("Chrome/")) {
				BROWSER_VERSION = str.substring(str.indexOf("Chrome/") + 7,
						str.lastIndexOf("Safari/"));
	...
	private static void getCapabilitiesDetails(WebDriver paramWebDriver) {
		Capabilities localCapabilities = ((RemoteWebDriver) paramWebDriver)
				.getCapabilities();
