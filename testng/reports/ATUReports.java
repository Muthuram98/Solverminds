...
public class ATUReports {
	...
	private static void stepFailureHandler(ITestResult paramITestResult,
			Steps paramSteps, LogAs paramLogAs) {
		if (paramLogAs == LogAs.FAILED) {
			buildReportData(paramSteps);
			if (Directory.continueExecutionAfterStepFailed)
				paramITestResult.setAttribute("passedButFailed",
						"passedButFailed");
			else
	...
	public static void add(String paramString, LogAs paramLogAs,
		...
		stepFailureHandler(Reporter.getCurrentTestResult(), localSteps,
				paramLogAs);
	}
	...
	public static void add(String paramString1, String paramString2,
			String paramString3, LogAs paramLogAs,
			CaptureScreen paramCaptureScreen) {
		if (paramCaptureScreen != null)
			if (paramCaptureScreen.isCaptureBrowserPage())
				takeBrowserPageScreenShot();
	...
	private static void takeDesktopScreenshot() {
		...
		File localFile = new File(str + Directory.SEP + screenShotNumber
				+ ".PNG");
		try {
			Rectangle localRectangle = new Rectangle(Toolkit
					.getDefaultToolkit().getScreenSize());
			BufferedImage localBufferedImage = new Robot()
					.createScreenCapture(localRectangle);
	...
	static {
		try {
			lastExecutionTime = Reporter.getCurrentTestResult()
					.getStartMillis();
		} catch (Exception localException) {
		}
	}
